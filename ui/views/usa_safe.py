...
class UsaSafeView(ttk.Frame):
    ...
    def build_ui(self):
        ...
        header_label = ttk.Label(
            self,
            text="USA Hockey Safe Sport Compliance Data",
        ...
        desc_label = ttk.Label(
            self,
            text="Records for Managers (M) and Volunteers (V) requiring Safe Sport compliance monitoring",
            font=("Helvetica", 10),
            foreground="gray"
        )
        desc_label.pack(pady=(0, 20))
        ...
        self.export_btn = ttk.Button(
            controls_frame,
            text="Export Safe Sport Data",
            command=self.export_safe_data,
    ...
    def create_key_stats_panel(self):
        """Create the Key Stats panel with count buttons."""
        # Key Stats Panel
        stats_frame = ttk.LabelFrame(self, text="Key Stats", padding=10)
        stats_frame.pack(fill=X, padx=20, pady=(0, 10))
        
        # Create buttons frame with 5 columns
        ...
        # Incomplete button
        self.incomplete_btn = ttk.Button(
            buttons_frame,
            text="Incomplete: 0",
            command=lambda: self.filter_by_safe_sport_status("Incomplete"),
            style="danger.TButton"
        )
        self.incomplete_btn.grid(row=0, column=0, padx=5, pady=5, sticky="ew")
        
        # <= 30 days button
        self.days_30_btn = ttk.Button(
            buttons_frame,
            text="<= 30 days: 0",
            command=lambda: self.filter_by_expiration_days(30),
            style="danger.TButton"
        )
        self.days_30_btn.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        # 30 < 90 days button
        self.days_30_90_btn = ttk.Button(
            buttons_frame,
            text="30 < 90: 0",
            command=lambda: self.filter_by_expiration_range(30, 90),
            style="warning.TButton"
        )
        self.days_30_90_btn.grid(row=0, column=2, padx=5, pady=5, sticky="ew")
        
        # >= 90 days button
        self.days_90_btn = ttk.Button(
            buttons_frame,
            text=">= 90 days: 0",
            command=lambda: self.filter_by_expiration_days(90, greater_than=True),
            style="success.TButton"
        )
        self.days_90_btn.grid(row=0, column=3, padx=5, pady=5, sticky="ew")
        
        # Complete button
        self.complete_btn = ttk.Button(
            buttons_frame,
            text="Complete: 0",
            command=lambda: self.filter_by_safe_sport_status("Complete"),
            style="success.TButton"
        )
        self.complete_btn.grid(row=0, column=4, padx=5, pady=5, sticky="ew")

    def update_key_stats(self):
        """Update the key stats buttons with current counts."""
        if self.filtered_data is None or self.filtered_data.empty:
            self.complete_btn.config(text="Complete: 0")
            self.incomplete_btn.config(text="Incomplete: 0")
            self.days_30_btn.config(text="<= 30 days: 0")
            self.days_90_btn.config(text="<= 90 days: 0")
            return
        
        # Count Complete records
        complete_count = 0
        if 'Safe Sport' in self.filtered_data.columns:
            complete_count = len(self.filtered_data[self.filtered_data['Safe Sport'] == 'Complete'])
        elif 'safe_sport' in self.filtered_data.columns:
            complete_count = len(self.filtered_data[self.filtered_data['safe_sport'] == 'Complete'])
        
        # Count Incomplete records
        incomplete_count = 0
        if 'Safe Sport' in self.filtered_data.columns:
            incomplete_count = len(self.filtered_data[self.filtered_data['Safe Sport'] == 'Incomplete'])
        elif 'safe_sport' in self.filtered_data.columns:
            incomplete_count = len(self.filtered_data[self.filtered_data['safe_sport'] == 'Incomplete'])
        
        # Count records expiring within 30 days
        days_30_count = self.count_expiring_within_days(30)
        
        # Count records expiring between 30 and 90 days
        days_30_90_count = self.count_expiring_in_range(30, 90)
        
        # Count records expiring within 90 days or more
        days_90_count = self.count_expiring_within_days(90, greater_than=True)
        
        # Update button texts
        self.complete_btn.config(text=f"Complete: {complete_count}")
        self.incomplete_btn.config(text=f"Incomplete: {incomplete_count}")
        self.days_30_btn.config(text=f"<= 30 days: {days_30_count}")
        self.days_30_90_btn.config(text=f"30 < 90: {days_30_90_count}")
        self.days_90_btn.config(text=f">= 90 days: {days_90_count}")

    def count_expiring_within_days(self, days, greater_than=False):
        """Count records expiring within the specified number of days."""
        if self.filtered_data is None or self.filtered_data.empty:
            return 0
        
        # Find the date column
        date_column = None
        if 'Date to Expire' in self.filtered_data.columns:
            date_column = 'Date to Expire'
        elif 'date_to_expire' in self.filtered_data.columns:
            date_column = 'date_to_expire'
        ...
        try:
            ...
            # Count records expiring within the specified days
            count = 0
            for _, row in self.filtered_data.iterrows():
                expire_date_str = row.get(date_column)
                if pd.isna(expire_date_str) or expire_date_str == "":
                    continue
                ...
                try:
                    ...
                    if isinstance(expire_date_str, str):
                        ...
                        for fmt in ['%m/%d/%Y', '%Y-%m-%d', '%m/%d/%y', '%Y-%m-%d %H:%M:%S']:
                            try:
                                expire_date = datetime.strptime(expire_date_str, fmt).date()
                                break
                            except ValueError:
                                continue
                        else:
                        ...
                        expire_date = expire_date_str.date()
                    
                    # Check if expiring within specified days
                    if greater_than:
                        # For >= 90 days, show records that expire 90+ days from now
                        if expire_date >= target_date:
                            count += 1
                    else:
                        # For <= 30 days, show records that expire within 30 days
                        if expire_date <= target_date:
                            count += 1
            ...
            logger.error(f"Error counting expiring records: {e}")
            return 0

    def count_expiring_in_range(self, min_days, max_days):
        """Count records expiring within a specific range of days."""
        if self.filtered_data is None or self.filtered_data.empty:
            return 0
        ...
        try:
            ...
            # Count records expiring within the specified range
            count = 0
            for _, row in self.filtered_data.iterrows():
                expire_date_str = row.get(date_column)
                if pd.isna(expire_date_str) or expire_date_str == "":
                ...
                try:
                    ...
                    # Check if expiring within the specified range
                    if min_date <= expire_date <= max_date:
                        count += 1
            ...
            logger.error(f"Error counting expiring records in range: {e}")
            return 0

    def filter_by_safe_sport_status(self, status):
        """Filter the table to show only records with the specified Safe Sport status."""
        if self.filtered_data is None or self.filtered_data.empty:
            return
        ...
        # Apply Safe Sport filter first
        if 'Member Type' in self.filtered_data.columns:
            self.filtered_data = self.filtered_data[
                self.filtered_data['Member Type'].isin(['M', 'V'])
            ]
        elif 'member_type' in self.filtered_data.columns:
            self.filtered_data = self.filtered_data[
                self.filtered_data['member_type'].isin(['M', 'V'])
            ]
        
        # Then apply status filter
        if 'Safe Sport' in self.filtered_data.columns:
            self.filtered_data = self.filtered_data[
                self.filtered_data['Safe Sport'] == status
            ]
        elif 'safe_sport' in self.filtered_data.columns:
            self.filtered_data = self.filtered_data[
                self.filtered_data['safe_sport'] == status
            ]
        ...
        # Update status
        self.data_status_var.set(f"Showing {status} Safe Sport records")

    def filter_by_expiration_days(self, days, greater_than=False):
        """Filter the table to show only records expiring within the specified days."""
        if self.filtered_data is None or self.filtered_data.empty:
            return
        ...
        if date_column is None:
            messagebox.showwarning("Warning", "No expiration date column found")
            return
        ...
        try:
            ...
            # Filter records expiring within the specified days
            filtered_indices = []
            ...
            # Apply the filter
            self.filtered_data = self.filtered_data.loc[filtered_indices]
            ...
            # Update status
            if greater_than:
                self.data_status_var.set(f"Showing records expiring in {days}+ days")
            else:
                self.data_status_var.set(f"Showing records expiring within {days} days")
            
        except Exception as e:
            logger.error(f"Error filtering by expiration days: {e}")
            messagebox.showerror("Error", f"Failed to filter by expiration days: {str(e)}")

    def filter_by_expiration_range(self, min_days, max_days):
        """Filter the table to show only records expiring within a specific range of days."""
        if self.filtered_data is None or self.filtered_data.empty:
            return
        ...
        try:
            ...
            # Filter records expiring within the specified range
            filtered_indices = []
            for idx, row in self.filtered_data.iterrows():
                expire_date_str = row.get(date_column)
                if pd.isna(expire_date_str) or expire_date_str == "":
            ...
            # Update status
            self.data_status_var.set(f"Showing records expiring between {min_days} and {max_days} days")
            
        except Exception as e:
            logger.error(f"Error filtering by expiration range: {e}")
            messagebox.showerror("Error", f"Failed to filter by expiration range: {str(e)}")
    ...
    def apply_safe_filter(self):
        """Apply Safe Sport filter: Member Type 'M' or 'V'."""
        if self.current_data is None:
            messagebox.showwarning("Warning", "No data to filter")
            return

        try:
            # Apply Safe Sport filter
            self.filtered_data = self.current_data.copy()
            
            # Filter for Member Type 'M' or 'V'
            if 'Member Type' in self.filtered_data.columns:
                self.filtered_data = self.filtered_data[
                    self.filtered_data['Member Type'].isin(['M', 'V'])
                ]
            ...
            logger.error(f"Error applying Safe Sport filter: {e}")
            messagebox.showerror("Error", f"Failed to apply Safe Sport filter: {str(e)}")

    def populate_table(self):
        """Populate the table with filtered data."""
        if self.filtered_data is None or self.filtered_data.empty:
            self.data_status_var.set("No Safe Sport records found")
        ...
        if saved_preferences:
            ...
            if self.visible_columns is None:
                # Prioritize important columns for Safe Sport workflow
                priority_columns = [
                    'Member ID', 'First Name', 'Last Name', 'Member Type', 
                    'Safe Sport', 'Date to Expire', 'Registration Date', 'Status'
                ]
    ...
    def update_display_with_filtered_data(self):
        ...
        # Define priority column settings for Safe Sport workflow
        priority_columns = {
            ...
            'Safe Sport': {'width': 100, 'minwidth': 80},
            'Date to Expire': {'width': 120, 'minwidth': 100},
            'Registration Date': {'width': 120, 'minwidth': 100},
        ...
        # Update status
        self.data_status_var.set("Safe Sport data loaded")
        self.record_count_var.set(f"Showing {displayed_records:,} Safe Sport records")
    ...
    def show_row_details(self, event):
        ...
        detail_window.title("Safe Sport Record Details")
    ...
    def export_safe_data(self):
        """Export the Safe Sport data to a file."""
        if self.filtered_data is None or self.filtered_data.empty:
            messagebox.showwarning("Warning", "No Safe Sport data to export")
            return

        # Ask user for export format and location
        file_path = filedialog.asksaveasfilename(
            title="Export Safe Sport Data",
        ...
        if file_path:
            try:
                ...
                if success:
                    messagebox.showinfo("Success", f"Safe Sport data exported successfully to:\n{output_path}")
                else:
                    messagebox.showerror("Error", "Failed to export Safe Sport data")
                    
            except Exception as e:
                logger.error(f"Export error: {e}")
                messagebox.showerror("Error", f"Failed to export Safe Sport data: {str(e)}")
    ...
    def load_column_visibility_preferences(self):
        ...
        try:
            ...
            if preferences_path.exists():
                ...
                return preferences.get('usa_safe_column_visibility', {})
        except Exception as e:
    ...
    def save_column_visibility_preferences(self, column_visibility):
        ...
        try:
            ...
            preferences['usa_safe_column_visibility'] = column_visibility
